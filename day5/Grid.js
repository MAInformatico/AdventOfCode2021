class Grid {
	...
	constructor({ Factory: Factory = (x, y) => 0, stringMap = {} } = {}) {
		this.Factory = Factory.bind(this);
	...
	static toCoords(id, returnAsObject = false) {
		let [x, y] = id.split(',');
		x = parseInt(x, 10);
		y = parseInt(y, 10);
	...
	neighbors(x, y) {
		...
		const neighborsLookup = [
			['N', [x, y - 1]], ['W', [x - 1, y]], ['E', [x + 1, y]], ['S', [x, y + 1]],
		];
	...
	clone() {
		...
		gridClone.stringMap = JSON.parse(
			JSON.stringify(this.stringMap)
		);
